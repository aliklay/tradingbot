...
model = DQNAgent.create_dqn_model(input_shape, n_actions, timesteps=1)
agent = DQNAgent(model, n_actions, epsilon_decay=0.995, gamma=0.99, batch_size=64, update_freq=1000, buffer_size=10000)
...
for episode in range(n_episodes):
    ...
    print(f"Episode {episode + 1}/{n_episodes} completed with reward {episode_reward}")
...
best_model = DQNAgent.create_dqn_model(input_shape, n_actions)
best_model.load_weights(model_weights_filepath)
